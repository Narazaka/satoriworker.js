...
(function () {
  ...
  function workerRoutine() {
    var Satori = require("libsatori.js"),
  ...
  function clientRoutine(workerMaker) {
    var SatoriWorker = function (_NativeShioriWorkerCl) {
      _inherits(SatoriWorker, _NativeShioriWorkerCl);

      _createClass(SatoriWorker, [{
        key: 'worker',
        get: function get() {
          return this._worker;
        }
      ...
      function SatoriWorker(fs, workerErrorHandler) {
    ...
    if (typeof window !== 'undefined') {
      window.SatoriWorker = SatoriWorker;
      if (window.ShioriLoader) {
        window.ShioriLoader.shiories.satori = SatoriWorker;
      } else {
        throw new Error("load ShioriLoader first");
      }
    } else {
      // node.js
      module.exports = SatoriWorker;
      if (typeof require !== 'undefined') {
        var ShioriLoader = require('shioriloader');
        ShioriLoader.shiories.satori = SatoriWorker;
      }
    }
  }
